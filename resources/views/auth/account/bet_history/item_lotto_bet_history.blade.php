...
@endphp<div class="item p-l-5 p-r-5 p-t-5">
    <div class="c-row c-row-between c-row-middle info">
        <div class="name c-row" style="border-radius: 5px;">
            <div class="type">{{isset($item->gameLottoType) ? Support::show($item->gameLottoType->gameLottoCategory,'name').': ':''}}{{Support::show($item->gameLottoType,'name')}}</div>
        </div>
        <div class="price c-row">
            <div class="OrderNumber c-tr">{{Support::show($item,'game_lotto_play_record_id')}}</div>
            <div class="m-l-10 tag-read van-image copy-text-btn" data-clipboard-text="{{Support::show($item,'game_lotto_play_record_id')}}" style="width: 18px; height: 15px;">
