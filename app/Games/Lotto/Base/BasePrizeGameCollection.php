...
namespace App\Games\Lotto\Base;
...
abstract class BasePrizeGameCollection extends ALottoable
{
    ...
    {
        $results =  $this->gameLottoProvider->getGameTableResult()::select('type_prize', 'value')->where('game_lotto_play_record_id', $this->currentGameRecord->id)->get()->groupBy('type_prize')->toArray();
        ...
        foreach ($this->gameLottoPlayUserBets as $userBet) {
            ...
            if (($count = count($numberWins)) > 0) {
                ...
                $userBet->game_lotto_play_user_bet_status_id =  $this->gameLottoProvider->getGameUserBet()::STATUS_WIN;
            } else {
                $userBet->game_lotto_play_user_bet_status_id = $this->gameLottoProvider->getGameUserBet()::STATUS_LOSE;
            }
    ...
    protected function standardMaxAppear()
    {
    ...
    {
        $GENERATOR = $this->gameLottoProvider->getGenerator();
        $generator = new $GENERATOR($this->currentGameRecord, $this->maxAppearIncludeNumbers, $this->mixIncludeNumbers, $this->mixExcludeNumbers);
        foreach ($this->listPrizeGames as $game) {
    ...
    protected function calculateMaxAppearIncludeNumbers($conditions)
    {
    ...
    protected function mixExcludeNumbers($conditions)
    {
        foreach ($conditions as $condition) {
            $numbers = $condition->getNumbers();
    ...
    protected function mixIncludeNumbers($conditions)
    {
        foreach ($conditions as $condition) {
            $numbers = $condition->getNumbers();
    ...
    protected function addGameToList($gameBet)
    {
