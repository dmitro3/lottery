var GAMEINFO_LOTTO = (function () {
    ...
    var initGameTime = function (second) {
        ...
        function gameTimer() {
            ...
            if (anchorTime <= 0) {
                $(".lotto-current-item-wrapper").addClass("in-loading-item");
                if (timeIntevalCurrentGame) {
                    clearInterval(timeIntevalCurrentGame);
                }
    ...
    var initCurrentGame = function () {
        $.ajax({
            url: "esystem/game-info/lotto?action=load_current_game",
            method: "get",
            global: false,
            dataType: "json",
        }).done(function (data) {
            ...
            $(".lotto-current-item-result").html(data.html);
            $(".lotto-current-item-wrapper").removeClass("in-loading-item");
    ...
    var initCurrentGameTypeHistory = function () {
        $(".lotto-history-list-item-result").addClass("in-loading-item");
        $.ajax({
            url: "esystem/game-info/lotto?action=load_current_game_type_history",
            method: "get",
            global: false,
            dataType: "html",
        }).done(function (data) {
            $(".lotto-history-list-item-result").html(data);
            $(".lotto-history-list-item-result").removeClass("in-loading-item");
        });
    };
    var initCurrentGameTypeHistoryPaginate = function () {
        $(document).on(
            "click",
            ".lotto-history-list-item-result .pagination a",
            function (e) {
                e.preventDefault();
                $(".lotto-history-list-item-result").addClass(
                    "in-loading-item"
                );
                $.ajax({
                    url: $(this).attr("href"),
                    ...
                    $(".lotto-history-list-item-result").html(data);
                    $(".lotto-history-list-item-result").removeClass(
                        "in-loading-item"
                    );
                });
            }
    ...
    var initShowHistoryContent = function () {
        $(document).on(
            "click",
            ".lotto-history-list-item .btn-show-hidden-content",
            function () {
                $(".lotto-item-history-hidden-content").slideUp(300);
                $(this).toggleClass("active");
                if ($(this).hasClass("active")) {
                    $(this)
                        .closest(".item")
                        .find(".lotto-item-history-hidden-content")
                        .slideDown(300);
                }
            }
        );
