...
export default class GameSelect {
    ...
    initEvents() {
        let self = this;
        Selector.on('click', '.ls_number .group_number button.btn_xs', function (e: any) {
            var _this = e.target;
        ...
        Selector.on('click', '.ls_number .item_number_select span', function (e: any) {
            var _this = e.target;
    ...
    addNumber(panel: any, number: any) {
        let listChoosen = panel.querySelector('.ls_number .ls_number.list_choosen');
        ...
        if (item) {
            ...
            <span type="checkbox" data-number="${number}">${number}</span>
        </label>`;
