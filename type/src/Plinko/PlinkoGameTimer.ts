...
import PlinkoSocket from "./PlinkoSocket";
...
export default class PlinkoGameTimer {
    ...
    public countDownPlayBox(status: boolean) {
        if (!status) {
    ...
    public showTimePlayBox() {
        let lastPoint = parseInt(PLINKO_CONFIG.LAST_POINT_TO_BET);
        let status = Storage.isGameBetted() || this.timeRemaining < lastPoint;
