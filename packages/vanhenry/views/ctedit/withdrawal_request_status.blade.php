...
@if ($dataItem->status_changed == 1)
    <div class="form-group">
        ...
        <p style="width: 100%;pointer-events: none;" class="btn py-1 btn-{{$realItem->withdrawal_request_status_id == WithdrawalRequestStatus::STATUS_CONFIRMED ? 'success':''}}{{$realItem->withdrawal_request_status_id == WithdrawalRequestStatus::STATUS_CANCEL ? 'danger':''}}">{{Support::show($realItem->withdrawalRequestStatus,'name')}}</p>
    </div>
@else
