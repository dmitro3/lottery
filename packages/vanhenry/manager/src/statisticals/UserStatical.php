...
class UserStatical
{
    public static function getUserTotalCollect($user)
    {
        ...
        return $userWallet->walletHistory()->where('type',WalletTransactionType::RECHARGE_MONEY)->sum('amount');
    }
    public static function getUserTotalSpend($user)
    {
        ...
        $userWallet = $user->getWallet();
        $total = $userWallet->walletHistory()->whereIn('type',$arrTypeSpend)->sum('amount');
        return abs($total);
    }
}
